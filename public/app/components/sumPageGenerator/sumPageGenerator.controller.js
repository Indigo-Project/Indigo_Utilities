app.controller("SumPageGenerator", ['$scope', '$state', '$http', 'siteNavigation', 'TTI_API', 'socket', function($scope, $state, $http, siteNavigation, TTI_API, socket) {
  ...
  $scope.data.sumPageDownload = function() {
          ...
          .then(function(data2) {
            if (data2.data.error) {
              ...
              if(confirm("download " + filteredReportList.length + " reports from '" + linkInfo.link.name + "' to " + data2.data + "?")) {
                ...
                TTI_API.sumPageDownload($scope.form.login, $scope.form.password, $scope.form.accountID, $scope.form.linkID, $scope.form.directory, filteredReportList, reportTypesUserOutput)
                .then(function(data) {
                  console.log(data);
                  $scope.view.formStatus = "Download";
